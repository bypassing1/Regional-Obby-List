document.addEventListener('DOMContentLoaded', function () {
    ...
    function fetchData(prefix) {
        return fetch(`/api/getData?prefix=${prefix}`).then(response => response.json());
    }
    
    Promise.all([
        fetchData(globaldataPrefix),
        fetchData(playerstatPrefix),
        ...prefixList.map(prefix => 
            fetchData(prefix).then(listData => {
                if (listData.length > 0) {
                    const topVerifier = listData[0].verifier;
                    topVerifiersString += `${topVerifier}ㅤㅤ`; 
                }
                return listData; // optional if you want to use it later
            })
        )
    ])
